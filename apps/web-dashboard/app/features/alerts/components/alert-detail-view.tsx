...
export function AlertDetailView() {
  ...
  const asset = (assetsQuery.data ?? []).find(
    (item) => item.assetId === alert?.assetId,
  );

  if (alertQuery.isLoading || assetsQuery.isLoading)
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_23rem]">
        <div className="space-y-6">
          ...
          {alert.sourceType === "HEALTH_CHECK" ? (
            <HealthContext alert={alert} />
          ) : (
            <MetricContext alert={alert} />
          )}
        </div>
        <div className="space-y-6">
    ...
    "
              ...
              {alert.status === "RESOLVED" && (
                <Button
                  ...
                  aria-busy={closeMutation.isPending}
              ...
              {alert.status === "ACKNOWLEDGED" && (
                <p className="rounded-md bg-slate-100 p-3 text-xs text-slate-600">
                  Acknowledged. The alert will resolve automatically when the
                  signal recovers.
                </p>
              )}
              {alert.status === "CLOSED" && (
  ...
  return (
    <Card className="border-slate-200 shadow-none">
      <CardHeader className="border-b border-slate-200">
        ...
        <p className="text-xs text-slate-500">
          Events that actually occurred for this alert
        </p>
      </CardHeader>
      <CardContent className="p-5">
        <ol>
          {events.map((event, index) => (
            <li
              key={event.lifecycleEventId}
              className="relative flex gap-3 pb-6 last:pb-0"
            >
              {index < events.length - 1 && (
                <span className="absolute left-2 top-4 h-full w-px bg-blue-300" />
              )}
              <span className="relative z-10 mt-0.5 flex size-4 items-center justify-center rounded-full bg-blue-600 text-white">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  {eventDescription(event, alert, assetName)}
                </p>
              </div>
            </li>
          ))}
...
function HealthContext({ alert }: { alert: Alert }) {
  ...
  return (
    <Card className="border-slate-200 shadow-none">
      <CardHeader className="border-b border-slate-200">
        <CardTitle className="text-sm">Health check context</CardTitle>
        <p className="text-xs text-slate-500">
          Endpoint result that produced this alert
        </p>
      </CardHeader>
      <CardContent className="space-y-4 p-5">
        <div className="grid gap-4 sm:grid-cols-3">
          <Info label="URL" value={url ?? "—"} />
          <Info label="HTTP" value={contextValue(alert, "statusCode")} />
          <Info
            label="Response"
            value={
              alert.context?.responseTimeMs == null
                ? "—"
                : `${String(alert.context.responseTimeMs)} ms`
            }
          />
        </div>
        {alert.context?.error != null && (
          <p className="rounded-md border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">
            {String(alert.context.error)}
          </p>
        )}
        <Link
          href={`/health-checks/${alert.sourceId}`}
          className="
        ...
        markLine:
          alert.thresholdValue == null
            ...
            : {
                ...
                data: [
                  {
                    yAxis: alert.thresholdValue,
                    name: `Threshold ${alert.thresholdValue}%`,
                  },
                ],
              },
      },
    ...
    <Card className="border-slate-200 shadow-none">
      ...
      <CardContent className="p-4">
        {metricQuery.isLoading ? (
          <Message text="Loading metric context..." />
        ) : metricQuery.isError ? (
          <Message text="Metric context is unavailable." destructive />
        ) : points.length === 0 ? (
          <Message text="No time-series context is available for this alert." />
        ) : (
...
function Details({ alert, assetName }: { alert: Alert; assetName?: string }) {
  const rows = [
    ...
    [
      "Signal",
      alert.metricType === "CPU_USAGE"
        ? "CPU Usage"
        : alert.metricType === "MEMORY_USAGE"
          ? "Memory Usage"
          : alert.metricType === "DISK_USAGE"
            ? "Disk Usage"
    ...
    [
      "Actual",
      alert.actualText ??
        (alert.actualValue == null ? "—" : String(alert.actualValue)),
    ],
    [
      "Threshold",
      alert.thresholdValue == null ? "—" : `${alert.thresholdValue}%`,
    ],
    ...
    [
      "Closed",
      alert.closedAt ? new Date(alert.closedAt).toLocaleString() : "—",
    ],
  ];
  return (
    <Card className="border-slate-200 shadow-none">
      ...
      <CardContent className="space-y-3 p-4">
        {rows.map(([label, value]) => (
          ...
          >
            ...
            <span className="max-w-52 break-words text-right font-medium">
              {value}
            </span>
          </div>
        ))}
...
function eventTitle(event: AlertLifecycleEvent): string {
  if (event.eventType === "RESOLVED" && !event.actorUserId)
...
function eventDescription(
  ...
  if (event.eventType === "ACKNOWLEDGED")
    return "Acknowledged manually by an operator.";
  if (event.eventType === "CLOSED")
    return "Closed manually after the alert was resolved.";

  if (alert.sourceType === "METRIC_RULE") {
    const metric = alert.metricType.replaceAll("_", " ").toLowerCase();

    if (event.eventType === "TRIGGERED") {
      const parsed = event.reason?.match(
        /:\s*([\d.]+)%\s*>=\s*([\d.]+)%\s*for\s*(\d+)s/i,
      );
      if (parsed)
        return `${metric} reached ${parsed[1]}%, above the ${parsed[2]}% threshold for ${parsed[3]}s.`;
  ...
  return (
    ...
    (event.eventType === "RESOLVED"
      ? "The monitored endpoint recovered."
      : "Alert state changed.")
  );
}
function contextValue(alert: Alert, key: string) {
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      ...
      <p className="mt-2 break-words font-mono text-xs font-medium">{value}</p>
    </div>
  );
}
function Message({
  ...
  return (
    <div
      className={`py-12 text-center text-sm ${destructive ? "text-rose-600" : "text-slate-500"}`}
    >
...
function AlertDetailSkeleton() {
  return (
    <section className="space-y-6" aria-label="Loading alert details">
