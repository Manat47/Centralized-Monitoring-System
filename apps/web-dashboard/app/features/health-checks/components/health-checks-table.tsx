...
export function HealthChecksTable() {
  ...
  const [resultFilter, setResultFilter] = useState<"ALL" | HealthResultStatus>(
    "ALL",
  );
  const [archiveFilter, setArchiveFilter] = useState<
    "CURRENT" | "ARCHIVED" | "ALL"
  >("CURRENT");
  const [editingTarget, setEditingTarget] = useState<HealthCheckTarget | null>(
    null,
  );
  const [archiveTarget, setArchiveTarget] = useState<HealthCheckTarget | null>(
    null,
  );
        ...
        <CardContent className="p-0">
          <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 p-4">
            ...
            <Select
              value={resultFilter}
              onValueChange={(value) =>
                setResultFilter((value ?? "ALL") as typeof resultFilter)
              }
            >
            ...
            <Select
              value={archiveFilter}
              onValueChange={(value) =>
                setArchiveFilter((value ?? "CURRENT") as typeof archiveFilter)
              }
            >
            ...
            <Button
              ...
              variant="ghost"
              disabled={
                !search && resultFilter === "ALL" && archiveFilter === "CURRENT"
              }
              onClick={() => {
                setSearch("");
                setResultFilter("ALL");
            ...
            >
              {filteredTargets.length} of {targets.length} checks
              {(targetsQuery.isFetching || assetsQuery.isFetching) &&
                " · Updating"}
            </span>
          </div>
                      ...
                      >
                        ...
                        <TableCell>
                          <div className="flex max-w-md items-center gap-2">
                            <Link
                              href={`/health-checks/${target.healthCheckTargetId}`}
                              className="truncate rounded-sm font-mono text-xs text-blue-700 transition-colors duration-150 hover:text-blue-800 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40"
                            >
                              {target.url}
                            </Link>
                            {mismatch && (
                              <AlertTriangle
                                ...
                                aria-label="Origin differs from application endpoint"
                              />
                            )}
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          {target.latest
                            ? `${target.latest.responseTimeMs} ms`
                            : "—"}
                        </TableCell>
                        <TableCell>
                          {formatRelativeDate(
                            target.latest?.timestamp ?? target.lastCheckedAt,
                          )}
                        </TableCell>
                          ...
                          <AdminOnly>
                            <MenuPrimitive.Root>
                              <MenuPrimitive.Trigger
                                aria-label={`Actions for ${asset?.name ?? "health check"}`}
                                disabled={pending}
                                className="inline-flex size-7 items-center justify-center rounded-md text-slate-500 outline-none hover:bg-slate-100 focus-visible:ring-2 focus-visible:ring-blue-500 disabled:opacity-50"
                              >
                                {pending ? (
                              ...
                              <MenuPrimitive.Portal>
                                <MenuPrimitive.Positioner
                                  ...
                                  className="z-50"
                                >
                                  <MenuPrimitive.Popup className="min-w-44 rounded-md bg-white p-1 text-sm shadow-md ring-1 ring-slate-200 outline-none">
                                    <MenuItem
                                      icon={RefreshCw}
                                      label="Check now"
                                      disabled={
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogTitle>Archive health check?</AlertDialogTitle>

            <AlertDialogDescription>
              This health check will stop running, but its existing history will
              remain available.
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                ...
                archiveMutation.mutate(archiveTarget.healthCheckTargetId, {
...
function MenuItem({
  ...
  return (
    <MenuPrimitive.Item
      disabled={disabled}
      onClick={onClick}
      className={`flex cursor-default items-center gap-2 rounded px-2 py-2 outline-none data-highlighted:bg-slate-100 data-disabled:opacity-40 ${destructive ? "text-rose-600" : ""}`}
    >
    ...
    <Card className="overflow-hidden border-slate-200 shadow-none">
      <CardContent className="p-0">
        ...
        {Array.from({ length: 6 }).map((_, index) => (
          <div
            key={index}
            className="grid h-16 grid-cols-[1fr_1.5fr_0.8fr_0.8fr_0.7fr] items-center gap-5 border-t border-slate-100 px-4"
          >
...
function EditIntervalDialog({
  ...
  return (
    <Dialog open={Boolean(target)} onOpenChange={handleOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Edit check interval</DialogTitle>
          <DialogDescription>
            The endpoint URL remains unchanged to preserve history integrity.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid gap-2">
            <Label htmlFor="edit-health-interval">Interval (seconds)</Label>
            <Input
