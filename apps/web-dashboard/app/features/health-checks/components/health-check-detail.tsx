...
export function HealthCheckDetail() {
  ...
  if (targetsQuery.isLoading || assetsQuery.isLoading) return <Message text="Loading health check..." />;
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex justify-end"><Select value={String(rangeMinutes)} onValueChange={(value) => setRangeMinutes(Number(value ?? 60))}><SelectTrigger className="w-40 bg-white"><SelectValue>{rangeMinutes === 60 ? "Last 1 hour" : rangeMinutes === 360 ? "Last 6 hours" : "Last 24 hours"}</SelectValue></SelectTrigger><SelectContent><SelectItem value="60">Last 1 hour</SelectItem><SelectItem value="360">Last 6 hours</SelectItem><SelectItem value="1440">Last 24 hours</SelectItem></SelectContent></Select></div>
