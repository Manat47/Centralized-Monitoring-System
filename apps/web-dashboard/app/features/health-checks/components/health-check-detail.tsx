...
import {
  ...
  getHealthCheckReportSummary,
} from "../api/get-health-check-detail";
...
export function HealthCheckDetail() {
  ...
  const target = (targetsQuery.data ?? []).find(
    (item) => item.healthCheckTargetId === id,
  );
  ...
  const chartOption: EChartsOption = {
    ...
    yAxis: {
      type: "value",
      min: 0,
      axisLabel: { formatter: "{value}ms", color: "#64748b", fontSize: 11 },
    ...
    series: [
      {
        ...
        data: history.map((point) => [point.timestamp, point.responseTimeMs]),
      },
    ],
  };
    ...
    <section className="space-y-6">
      ...
      <Card className="border-slate-200 shadow-none">
        <CardContent className="grid gap-5 p-5 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <Info
            ...
            value={target.latest?.statusCode?.toString() ?? "—"}
          />
          <Info
            label="Response time"
            value={target.latest ? `${target.latest.responseTimeMs} ms` : "—"}
          />
        </CardContent>
      </Card>

      {hasOriginMismatch(asset?.endpoint ?? null, target.url) && (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          This health URL uses a different origin from the current application
          endpoint.
        </div>
      )}
      ...
      <div className="flex justify-stretch sm:justify-end">
        <Select
          ...
          onValueChange={(value) => setRangeMinutes(Number(value ?? 60))}
        >
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
        ...
        <Stat
          label="Avg response"
          value={
            summary?.responseTime.averageMs == null
              ? "—"
              : `${summary.responseTime.averageMs} ms`
          }
        />
        <Stat
          label="P95 response"
      ...
      <Card className="border-slate-200 shadow-none">
        <CardHeader className="border-b border-slate-200">
          ...
          <p className="text-xs text-slate-500">
            Historical response time for each check
          </p>
        </CardHeader>
        <CardContent className="p-4">
          {historyQuery.isLoading ? (
            <Message text="Loading response history..." />
          ) : history.length === 0 ? (
            <Message text="No checks in this time range." />
          ) : (
            <ReactECharts
              option={chartOption}
              style={{ height: 280 }}
      ...
      <Card className="overflow-hidden border-slate-200 shadow-none">
        <CardHeader className="border-b border-slate-200">
          ...
          <p className="text-xs text-slate-500">
            Most recent individual check results
          </p>
        </CardHeader>
                ...
                .map((point) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell>{point.statusCode ?? "—"}</TableCell>
                      <TableCell>{point.responseTimeMs} ms</TableCell>
                      <TableCell
                        className="max-w-md truncate text-xs text-rose-600"
...
function Info({
  ...
  return (
    <div className="min-w-0">
      ...
      <p
        className={`mt-2 truncate text-sm font-medium ${mono ? "font-mono text-xs" : ""}`}
        title={value}
      >
        {value}
      </p>
...
function Stat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50/50 px-4 py-3">
      ...
      <p className="mt-1.5 text-lg font-semibold tracking-tight tabular-nums text-slate-950">
        {value}
      </p>
    </div>
  );
...
function HealthCheckDetailSkeleton() {
  return (
    <section className="space-y-6" aria-label="Loading health check details">
